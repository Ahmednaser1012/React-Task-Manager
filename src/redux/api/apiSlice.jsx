...
export const fetchTasks = createAsyncThunk("tasks/fetchTasks", async () => {
  try {
    const response = await http.get("/tasks?order=created_at.desc");
...
export const updateTask = createAsyncThunk(
  ...
  async ({ id, updates }) => {
    try {
      const response = await http.patch(`/tasks?id=eq.${id}`, updates);
      return response.data[0];
    } catch (error) {
...
export const deleteTask = createAsyncThunk("tasks/deleteTask", async (id) => {
  try {
    await http.delete(`/tasks?id=eq.${id}`);
    return id;
  } catch (error) {
...
export const fetchCategoryById = createAsyncThunk(
  ...
  async (id) => {
    try {
      const response = await http.get(`/categories?id=eq.${id}`);
      return response.data[0];
    } catch (error) {
